...
class PyramidRescaler(torch.nn.Module):
    def __init__(self, num_channels, scales):
        super().__init__()
        for scale in scales:
        ...
        for n_steps in self.upscale_steps:
            ...
            if n_steps > 0:
                for _ in range(n_steps - 1):
                    step_operations.append(torch.nn.ConvTranspose2d(num_channels, num_channels, kernel_size=2, stride=2, bias=False))
                    step_operations.append(torch.nn.BatchNorm2d(num_channels))
                    step_operations.append(torch.nn.GELU())
                step_operations.append(torch.nn.ConvTranspose2d(num_channels, num_channels, kernel_size=2, stride=2))
            ...
            if n_steps < 0:
                for _ in range(-n_steps):
                    step_operations.append(torch.nn.MaxPool2d(kernel_size=2, stride=2))

            self.rescale_operations.append(torch.nn.Sequential(*step_operations))

    def forward(self, inputs):
        return [rescale_op(x) for rescale_op, x in zip(self.rescale_operations, inputs)]


class PPM(torch.nn.Module):
    def __init__(self, num_channels_in, num_channels_out, pooling_scales, dropout_rate=0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        out = self.conv_block(torch.cat([x, *pooled_x], dim=1))
        return out
...
class FPNBlock(torch.nn.Module):
    ...
    def forward(self, lateral_input, top_down_input):
...
class FPN(torch.nn.Module):
    def __init__(self, num_channels_in, num_channels_out, num_levels=4, ppm_scales=None, dropout_rate=0.1):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        for feature_map, fpn_block in zip(reversed(inputs[:-1]), self.fpn_blocks):
...
class FuseBlock(torch.nn.Module):
    def __init__(self, num_channels, num_layers, dropout_rate=0.1):
        super().__init__()
        self.conv_block = torch.nn.Sequential(
    ...
    def forward(self, inputs):
        ...
        out = self.conv_block(torch.cat(scaled_features, dim=1))
        return out



class UPerNet(torch.nn.Module):
    def __init__(self, num_backbone_features, num_classes, intermediate_layers, pyramid_scales, num_channels_fpn, ppm_scales):
        super().__init__()
    ...
    def forward(self, x):
        ...
        feature_pyramid = self.pyramid_rescaler(x)
...
class DINOUperNet(torch.nn.Module):
    def __init__(
            ...
            landcover_ckpt,
            solar_ckpt,
            landcover_out_name='sursentia_landcover',
            solar_out_name='sursentia_solar'):
        super().__init__()
